...
def new_max(grid, curr_max, y,x,dy,dx)
  return curr_max if y + 3*dy > 19 || y + 3*dy < 0 || x+dx*3 > 19 || x+dx*3 < 0
  prod = grid[y][x] * grid[y+dy][x+dx] * grid[y+dy*2][x+dx*2] * grid[y+dy*3][x+dx*3]
  prod > curr_max ? prod : curr_max
end

max = 0
for y in 0..19
  for x in 0..19
    max = new_max(grid, max, y, x, 0, 1)  # Right
    max = new_max(grid, max, y, x, 1, 0)  # Down
    max = new_max(grid, max, y, x, 1, 1)  # Down Right
    max = new_max(grid, max, y, x, 1, -1) # Down Left
  end
end
p max
